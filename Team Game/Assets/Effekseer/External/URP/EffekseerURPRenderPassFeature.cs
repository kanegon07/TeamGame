...
public class EffekseerURPRenderPassFeature : ScriptableRendererFeature
{
	class EffekseerRenderPassURP : UnityEngine.Rendering.Universal.ScriptableRenderPass
	{
		...
		public override void Execute(ScriptableRenderContext context, ref UnityEngine.Rendering.Universal.RenderingData renderingData)
		{
			...
			prop.colorTargetIdentifier = renderer.cameraColorTargetHandle.nameID;

			// 代替コード：RTHandleからのIdentifier取得
			RenderTargetIdentifier cameraDepthTargetIdentifier;
			if (renderer.cameraDepthTargetHandle != null)
			{
				cameraDepthTargetIdentifier = renderer.cameraDepthTargetHandle.nameID; // または rt など適切なプロパティを使用
			}
			else
			{
				cameraDepthTargetIdentifier = new RenderTargetIdentifier(BuiltinRenderTextureType.CameraTarget);
			}
			...
			var isValidDepth = IsValidCameraDepthTarget(cameraDepthTargetIdentifier);

			if (isValidDepth)
			{
				prop.depthTargetIdentifier = cameraDepthTargetIdentifier;
			}
			else
			{
